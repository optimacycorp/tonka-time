...
- On single-server deployments where the Tonka API and OpenSign both run in Docker, attach both stacks to a shared Docker network and set `OPENSIGN_INTERNAL_API_URL=http://opensign-server:8080/app` so the app container talks to the OpenSign server container directly instead of calling the public `sign.` hostname from the same box.
