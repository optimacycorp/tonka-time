...
- On single-server deployments where the Tonka API runs in Docker, set `OPENSIGN_INTERNAL_API_URL=http://host.docker.internal:8081/app` and map `host.docker.internal:host-gateway` in Compose so the app container talks to the RackNerd host's local OpenSign port directly instead of calling the public `sign.` hostname from the same box.
